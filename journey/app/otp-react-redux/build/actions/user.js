...
const setCurrentUser = (0, _reduxActions.createAction)('SET_CURRENT_USER');
const setCurrentUserMonitoredTrips = (0, _reduxActions.createAction)('SET_CURRENT_USER_MONITORED_TRIPS');
const setPathBeforeSignIn = (0, _reduxActions.createAction)('SET_PATH_BEFORE_SIGNIN');
...
function fetchUserMonitoredTrips(accessToken) {
  return async function (dispatch, getState) {
    ...
    const {
      otp_middleware: otpMiddleware = null
    } = otp.config.persistence;

    if (otpMiddleware) {
      const {
        data: trips,
        status: fetchStatus
      } = await (0, _middleware.getTrips)(otpMiddleware, accessToken);
...
function fetchOrInitializeUser(auth) {
  return async function (dispatch, getState) {
    ...
    if (otpMiddleware) {
      ...
      } = await (0, _middleware.fetchUser)(otpMiddleware, accessToken); // Beware! On AWS API gateway, if a user is not found in the middleware
  ...
  return async function (dispatch, getState) {
    ...
    if (otpMiddleware) {
      ...
      if ((0, _user.isNewUser)(loggedInUser)) {
        result = await (0, _middleware.addUser)(otpMiddleware, accessToken, userData);
      } else {
        result = await (0, _middleware.updateUser)(otpMiddleware, accessToken, userData);
...
function createOrUpdateUserMonitoredTrip(tripData, isNew, silentOnSuccess) {
  return async function (dispatch, getState) {
    ...
    if (otpMiddleware) {
      ...
      if (isNew) {
        result = await (0, _middleware.addTrip)(otpMiddleware, accessToken, tripData);
      } else {
        result = await (0, _middleware.updateTrip)(otpMiddleware, accessToken, tripData);
      } // TODO: improve the UI feedback messages for this.
...
function deleteUserMonitoredTrip(id) {
  return async function (dispatch, getState) {
    ...
    if (otpMiddleware) {
      ...
      const deleteResult = await (0, _middleware.deleteTrip)(otpMiddleware, accessToken, id);
